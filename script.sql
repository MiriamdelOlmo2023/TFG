...
CREATE TABLE MOVIMIENTOS (
    ...
    Nombre VARCHAR(100),
    PP INTEGER,
    Tipo VARCHAR(50) REFERENCES TIPOS(Tipo),
    Acc FLOAT,
    Poder INTEGER,
    Critico FLOAT,
    Base VARCHAR(10) CHECK (Base IN ('físico', 'especial', 'estado')),
    Efecto VARCHAR(255)
);
...
CREATE TABLE POKEMON (
    ...
    NOMBRE VARCHAR(60),
    GEN INTEGER,
    TIPO1 VARCHAR(50) REFERENCES TIPOS(Tipo),
    TIPO2 VARCHAR(50) REFERENCES TIPOS(Tipo),
    PS INTEGER,
    ATK INTEGER,
    DEF INTEGER,
    SPATK INTEGER,
    ...
    PESO INTEGER,
    CROMO VARCHAR(200),
    MOV1 INTEGER REFERENCES MOVIMIENTOS(ID),
    MOV2 INTEGER REFERENCES MOVIMIENTOS(ID),
    MOV3 INTEGER REFERENCES MOVIMIENTOS(ID),
    MOV4 INTEGER REFERENCES MOVIMIENTOS(ID),
    HAB INTEGER REFERENCES HABILIDAD(ID),
    PASIVA INTEGER REFERENCES PASIVA(ID)
);
